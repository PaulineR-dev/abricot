...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const assignedTasksCount = await prisma_1.default.task.count({
            where: {
                assignees: {
                    some: {
                        userId: authReq.user.id,
        ...
        const urgentTasksCount = await prisma_1.default.task.count({
            where: {
                assignees: {
                    some: {
                        userId: authReq.user.id,
        ...
        const overdueTasksCount = await prisma_1.default.task.count({
            where: {
                assignees: {
                    some: {
                        userId: authReq.user.id,
        ...
        const tasksByStatus = await prisma_1.default.task.groupBy({
            by: ["status"],
            where: {
                assignees: {
                    some: {
